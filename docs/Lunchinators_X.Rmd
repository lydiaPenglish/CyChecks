...
### Data modeling (help!)
***
- Variables: Position, gender, department, fiscal year

- Not every department has M & F employeees in all positions 
  - How do we include those departments with no gender representation...
...
- Is salary response variable and gender a fixed effect?
  - How to include department? 

```{r, eval = FALSE}

lmerTest::lmer(lsal ~ prof_simp + gender + (gender | dept), data = ex2, REML = F)
# singular fit
# not taking into account repeated measures

```


- Ratio of male:female salarys as response variable

```{r, eval = FALSE}
lmerTest::lmer(lrat ~ 1 + prof_simp + (1 | dept), data = lrat)
